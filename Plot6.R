...
balt_LA_moto = balt_LA[balt_LA$SCC %in% moto,]
require("ggplot2")
...
g + geom_line(stat="summary", fun.y="sum") + ylab(expression('Total PM'[2.5]*" emission from motor vehicle sources in Baltimore and LA")) + scale_colour_discrete(name  ="City", label=c("Los Angeles","Baltimore"))
dev.off()